...
from a8_model import VI_Block, VI_Untied_Block
...
tf.app.flags.DEFINE_integer('ch_q1',           4,                     'Channels in q layer (~transfer actions)')
tf.app.flags.DEFINE_integer('ch_q2',           4,                     'Channels in q layer (~new actions)')
...
tf.app.flags.DEFINE_string('logdir',          '/tmp/vin/g2/',          'Directory to store tensorboard summary')
...
with tf.Session() as sess:
    ...
    #restore pre-trained source model
    sess.run(init)
    var_to_restore = slim.get_variables_to_restore(include=['bias','w0','w1','w','w_fb'])
    saver = tf.train.Saver(var_to_restore)
    saver.restore(sess, 'result/VIN8.ckpt')
